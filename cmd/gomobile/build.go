...
func runBuild(cmd *command) (err error) {
	...
	if !nmpkgs["github.com/sumup/mobile/app"] {
		return fmt.Errorf(`%s does not import "github.com/sumup/mobile/app"`, pkg.ImportPath)
	}
...
func extractPkgs(nm string, path string) (map[string]bool, error) {
	if buildN {
		return map[string]bool{"github.com/sumup/mobile/app": true}, nil
	}
	r, w := io.Pipe()
...
func importsApp(pkg *build.Package) error {
	...
	for _, path := range pkg.Imports {
		if path == "github.com/sumup/mobile/app" {
			return nil
		}
	}
	return fmt.Errorf(`%s does not import "github.com/sumup/mobile/app"`, pkg.ImportPath)
