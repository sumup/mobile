...
func TestImportPackagesPathCleaning(t *testing.T) {
	...
	slashPath := "github.com/sumup/mobile/mobile/example/bind/hello/"
...
WORK=$WORK
GOOS=android CGO_ENABLED=1 gobind -lang=go,java -outdir=$WORK{{if .JavaPkg}} -javapkg={{.JavaPkg}}{{end}} github.com/sumup/mobile/mobile/asset
...
WORK=$WORK
GOOS=darwin CGO_ENABLED=1 gobind -lang=go,objc -outdir=$WORK -tags=ios{{if .Prefix}} -prefix={{.Prefix}}{{end}} github.com/sumup/mobile/mobile/asset
